...
export function getAllPatterns(savedPatterns: PatternDefinition[] = []): PatternDefinition[] {
  return [...BUILTIN_PATTERNS, ...savedPatterns];
}

export function getPatternById(
  patternId: string,
  savedPatterns: PatternDefinition[] = [],
): PatternDefinition | undefined {
  return getAllPatterns(savedPatterns).find((pattern) => pattern.id === patternId);
}
...
export function insertPatternAt(
  ...
  const pattern = getPatternById(patternId, savedPatterns);
